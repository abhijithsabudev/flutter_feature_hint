...
The `uniqueKey` is now mandatory, ensuring proper state tracking for the `playOnceInLifetime` feature.

```dart
FeatureHintOverlay(
  uniqueKey: 'delete_action_hint', // ⭐ REQUIRED
...
- Beautiful task manager UI with Material 3
- Dismissible list items with swipe animations
- Full-screen hint overlay (traditional)
- Bounded hint overlay (new!)
...
6. **State Persistence**: Records played animations in SharedPreferences
